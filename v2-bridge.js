...
(function () {
  ...
  // 1. Merge V2 demo into DEMO_ORDERS in-memory so the wallet & admin views can
  //    aggregate across both legacy and V2 demo orders.
  try {
    var origDemo = Array.isArray(window.DB.DEMO_ORDERS) ? window.DB.DEMO_ORDERS.slice() : [];
    var v2Demo = (Array.isArray(window.DB.DEMO_ORDERS_V2) ? window.DB.DEMO_ORDERS_V2 : []).map(v1Shape);
    var merged = origDemo.concat(v2Demo);
    Object.defineProperty(window.DB, 'DEMO_ORDERS', {
      value: merged, writable: true, configurable: true
    });
  } catch (e) {
    var origDemoF = Array.isArray(window.DB.DEMO_ORDERS) ? window.DB.DEMO_ORDERS.slice() : [];
    window.DB.DEMO_ORDERS = origDemoF.concat((window.DB.DEMO_ORDERS_V2 || []).map(v1Shape));
  }
